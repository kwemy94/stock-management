    ...
    const [form, setForm] = useState(() => ({
        client: invoice
            ? {
                  ...
                  label: `${invoice.customer.name} (${invoice.customer.phone})`,
                  ...invoice.customer,
              }
            : null,
        dateFacture: invoice
        ...
        modePaiement: invoice
            ? {
                  value: invoice.payments[0]?.mode_id,
                  label: invoice.payments[0]?.name,
                  ...invoice.payments,
              }
            : [],
    }));
    ...
    const handleSubmit = async (e, statut) => {
        ...
        try {
            ...
            if (res.status == 200) {
                ...
                let stat = statut == "proformat" ? "proforma" : statut;
                window.location.href = `/dashboard/sale-invoice?type=${stat}`;
            } else {
                    ...
                    <div className="mb-3 text-center">
                        <h5 className="mb-0 fw-semibold">
                            {invoice?.status === "proformat"
                                ? `Modification proforma N°  ${invoice.invoice_number}`
                                : `Modification commande N°  ${invoice.invoice_number}`}
                        </h5>
